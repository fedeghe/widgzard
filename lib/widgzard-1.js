...
(function (W){
    ...
    function Wnode(conf, trg, mapcnt) {
        ...
        // free the parent promise from waiting
        //
        ...
        //
        this.node.root = mapcnt.root;

        // save a reference to the parent
        // 
        this.node.parent = trg;
        ...
        // 
        this.node.WIDGZARD_promise = Promise.create();
     ...
     */
    Wproto.setData = function (node, data) {
        node.data = data || {};
        return this;
    };
    ...
    function render (params, clean) {
        ...
        (function recur(cnf, trg){
            ...
            // 
            if (cnf.content) {
                for (i = 0, l = cnf.content.length; i < l; i++) {

                    // replace i-th content if a clearer node is required via
                    // the 'clearer' alias
                    ...
                    // now recur on i-th child
                    // 
                    recur(
                        // the i-th content cnf
                        // 
                        cnf.content[i],

                        // the i-th content node is added to trg,
                        // which returns the added node, then passe as parent
                        // for the recursive call
                        // 
                        new Wnode(cnf.content[i], trg, mapcnt).add()
                    );
                }
            }
