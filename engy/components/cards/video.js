{
			...
			content : [{
				...
				cb : function () {
					...
					if(this.climb(3).data.controls) {
						...
						this.node.removeAttribute('autoplay');
	...
	cb : function () {
		...
		// Opera bug
		!self.data.autoplay && FG.events.on(v, 'loadeddata', function () {
			v.pause();
		});
		
		if (self.data.onEnd) {
			FG.events.on(v, 'ended', function () {
		...
		FG.Channel(cid).sub('move', moved);
		FG.Channel(cid).sub('moved', moved);
