SM = {
    ...
    content : [{
    	...
    	content : [{
    		...
    		cb : function () {
                ...
                if (parent.data.orientation == 'vertical') {
                    FG.dom.removeClass(self.node, 'roundright');
                    FG.dom.addClass(self.node, 'roundtop');
                }

                if (self.data.layout) {
                    FG.dom.addClass(self.node, self.data.layout);
                } 

                FG.Channel(parent.data.id).sub('moved', function () {
                    if (parent.data.index > 0) {
                        FG.dom.removeClass(self.node, 'disabled');
                    } else {
                        FG.dom.addClass(self.node, 'disabled');       
                    }
                });
    		...
    		content : [{
    			...
    			cb : function () {
					...
					for (var i = 0; i < len; i++){
						FG.dom.addClass(childs[i].node, orientation);
        ...
        cb : function () {
            var self = this;
            FG.dom.addClass(self.node, self.parent.data.orientation);
            self.done();
        }
    }],
    cb : function (){
        ...
        FG.Channel(data.id).sub('moved', function (topic, from, to) {
            previous = to;
        });
        ...
        FG.dom.addClass(self.node ,'coverBgHigh');

        self.node.addEventListener('click', function (e) {
        	if (FG.events.eventTarget(e).parentNode == self.node) {
        ...
        FG.Channel(data.id).sub('move', function (topic, dir) {
            ...
            FG.Channel(data.id).pub('moved', [previous, data.index]);
                ...
                content : [{
                    ...
                    content : [{
                        ...
                        cb : function () {
                            ...
                            FG.events.on(self.node, 'click', function (e) {
                                var trg = FG.events.eventTarget(e),
                                    n,
                                    move;

                                FG.dom.removeClass(self.node.children.item(data.index), 'active');
                                ...
                                FG.dom.addClass(self.node.children.item(n), 'active');
                            });

                            FG.Channel(data.id).sub('moved', function (topic, from, to) {
                                FG.dom.removeClass(self.node.children.item(from), 'active');
                                FG.dom.addClass(self.node.children.item(to), 'active');
                            });
        ...
        timeEnd = +new Date;
        alert('TTR ' + (timeEnd - timeStart) + 'ms');
